...
public class GetAssessment : ICarterModule
{
    ...
    public record GetAssessmentResponse(int Id, int IdDataCut, int IdFunctionary, int IdPatient, string YearOld, DateTime Date, string Eps, int IdUserCreated, int IdUserUpdate, DateTime UpdateDate, DateTime CreateDate, List<ValuationModel> Valuations);
    ...
    public class GetAssessmentHandler(AppDbContext context, IValidator<GetAssessmentQuery> validator) : IRequestHandler<GetAssessmentQuery, IResult>
    {
        public async Task<IResult> Handle(GetAssessmentQuery request, CancellationToken cancellationToken)
        {
            ...
            var assessment = await context.Assessments.Include(x => x.Valuations).Where(x => x.Id == request.Id).FirstOrDefaultAsync(cancellationToken);
