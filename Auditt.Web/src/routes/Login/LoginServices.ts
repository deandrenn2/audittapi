...
export const createUserServices = async (
	...
	const url = "api/users";
	const response = await ApiClient.post<MsgResponse<CreateUserModel>>(
		url,
		model
	);
	if (response.status !== 200) {
...
): Promise<MsgResponse<CreateBusinessModel>> => {
	const url = "api/businesses";
	const response = await ApiClient.post<MsgResponse<CreateBusinessModel>>(
		url,
		model
	);
	if (response.status !== 200) {
...
export const PasswordRecover = async (
	model: passwordRecoverModel
): Promise<MsgResponse<passwordRecoverModel>> => {
	const url = "api/users/securePharse";
	const response = await ApiClient.post<MsgResponse<passwordRecoverModel>>(
		url,
		model
	);
	if (response.status !== 200) {
...
export const updatePassword = async (
	model: UsersUpdatePasswordModel
): Promise<MsgResponse<UsersUpdatePasswordModel>> => {
	const url = `api/users/password`;
	const response = await ApiClient.post<MsgResponse<UsersUpdatePasswordModel>>(
		url,
		model
	);
